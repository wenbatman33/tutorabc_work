$(function () {
	//固定 landing from
	var speed=500;
	var position = $('.regFixedWrap').position();
    ...
    if (scroll_top > bottomLine) {
        $('.regForm').stop().animate({'top':scroll_bottom },speed );
        $('.regCustomerServiceIcon').stop().animate({'top':scroll_bottom},speed );

    }else if (scroll_top > upLine) {
        $('.regForm').stop().animate({'top':scroll_top-580},speed );
        $('.regCustomerServiceIcon').stop().animate({'top':scroll_top-580},speed );
    }else{
       $('.regForm').stop().animate({'top':0},speed );
