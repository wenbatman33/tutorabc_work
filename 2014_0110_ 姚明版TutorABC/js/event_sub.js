$(function () {
	//固定 landing from
	var position = $('.regFixedWrapPages').position();
    ...
    var sticky_navigation = function () {
		...
		var scroll_bottom = $(document).height() - $('.regForm').height()-sticky_navigation_offset_top - $('.footer').height();

    if (scroll_top > bottomLine) {
        $('.regFormWrap').stop().animate({'top':scroll_bottom-20 },speed );
        $('.regCustomerServiceIcon').stop().animate({'top':scroll_bottom-20 },speed );

    }else if (scroll_top > upLine) {
        $('.regFormWrap').stop().animate({'top':scroll_top-270},speed );
        $('.regCustomerServiceIcon').stop().animate({'top':scroll_top-270},speed );
    }else{
       $('.regFormWrap').stop().animate({'top':0},speed );
       $('.regCustomerServiceIcon').stop().animate({'top':0},speed );
    }
