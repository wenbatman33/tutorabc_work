...
if currentDateTime > newUIOnlineDateTime then
	redirectFileName = "session_feedback_new_JR.asp"
else
...
str_sql = " SELECT TOP 1 CONVERT(VARCHAR(8), ClassStartDate, 112) + RIGHT('00' + ISNULL(CONVERT(VARCHAR(2), ClassStartTime),''),2) " 
str_sql = str_sql & " + RIGHT('000' + ISNULL(CONVERT(VARCHAR(3), ClassRoomNumber),''),3) AS session_sn, ISNULL(ProfileLearnLevel,0) AS attend_level, ClassRecordId, LobbySessionId " 
str_sql = str_sql & " FROM member.ClassRecord WITH (NOLOCK) "   
str_sql = str_sql & " WHERE client_sn=@client_sn AND ClassStartDate=@ClassStartDate AND ClassStartTime = @ClassStartTime AND ( ClassStatusId = 2 OR ClassStatusId = 3 ) AND ClassRecordValid = 1 "   
str_sql = str_sql & " ORDER BY ClassRecordId DESC "
...
if (isSelectQuerySuccess(arr_result)) then
	...
	if (Ubound(arr_result) >= 0) then
		...
		intClassRecordId = arr_result(2,0)
		LobbySessionId = arr_result(3,0)
        		
		'------- 更新進教室狀況 ------- start -------
		var_class_arr = Array(3,  intClassRecordId)
        strSql = " UPDATE member.ClassRecord SET " 
        strSql = strSql & " ClassStatusId = @ClassStatusId "
        strSql = strSql & " WHERE (ClassRecordId = @ClassRecordId) "
		arr_class_result = excuteSqlStatementWrite(strSql, var_class_arr, CONST_VIPABC_RW_CONN)
		'------- 更新進教室狀況 ------- end ---------
				
		'------tutormeet------ start -----
		str_attend_room = "jsession"&str_room
		'注意：要測試時，記得看Tutormeet 是否有資料
		'取教室亂數
		if (str_session_sn <> "") then 
			str_room_rand = GetTutorMeetVar(str_attend_room,str_session_sn,"jr")	
		end if 
		'檢查TutorMeet是否有此人
		'str_chk_member = getTutorMeetMember(g_var_client_sn, "vipjr", "1")	
				
		if ( sCInt(LobbySessionId) > 0 ) then
			str_class_type = 8 'For 大會堂
            strCompStatus = "abc-vipjr"
		else
			str_class_type = 1 'For regular , 1-1
            strCompStatus = "jr-vipjr"
		end if
	else
	'錯誤訊息
...
if ( true = bolDebugMode ) then
	...
	response.write "str_attend_room:" & str_attend_room & "<br/>"
	response.write "str_room_rand:" & str_room_rand & "<br/>"
	response.write "str_chk_member:" & str_chk_member & "<br/>"
...
function getSessionCode() {
	...
	var service_code = encodeTutorMeetInputParams("<%=str_session_sn%>","<%=g_var_client_sn%>","<%=str_attend_room%>","<%=str_room_rand%>",<%=str_class_type%>,"","<%=strCompStatus%>");
	<% end if %>
	return service_code;
}
