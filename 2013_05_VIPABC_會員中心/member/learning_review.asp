...
<script type="text/javascript" src="/lib/javascript/JQuery/Scripts/plugin/jquery.blockUI_2.57.js"></script>
...
<!--
<%
if request("shortsessionemail") <> "" and session("client_sn") = "" then	
	arrParam = array(request("shortsessionemail"))
	strSql =  " SELECT sn FROM dbo.client_basic WITH (NOLOCK) WHERE email =@email "
	Set objclientsn = excuteSqlStatementReadEach(strSql , arrParam, CONST_TUTORABC_RW_CONN)
	if ( not objclientsn.eof ) then
		session("client_sn") = objclientsn("sn")	
	end if 		
end if 
if request("shortsessionemail") <> "" and request("session_sn") <> "" and session("client_sn") <> "" then
%>
loadHomeWork("<%=request("session_sn")%>", "<%=session("client_sn")%>", undefined);
<% end if %>

//將測驗卷檔案讀取近來
function loadHomeWork(session_sn, intClientSn, p_index) 
{
	if(undefined != p_index)
	{
...
//alert(session_sn+"#"+intClientSn)
    $.ajax({
        ...
        error: function (xhr) { alert("錯誤"+xhr.responseText);/*alert("此課程為測試課程，請選擇正式課程1");*/ },
        success: function (msg) {
            //alert(msg);
            $(".main_membox").html(msg)
        },
...
function OpenMsg(strRecordName, intClientSn, intRecordType, intCostSession, intRecordSn, intMaterialSn, intVideoType, intControlType) {
    ...
    $.ajax({
        ...
        success: function (result) {
            if ("free" == result) { //表時間內觀看為免費
                $("#show_cost_session").text("0");
                $("#button").attr("value", "免费回顾");
            }
            else {
                $("#show_cost_session").text(result);
...
'20130320 阿捨新增 是否擁有贈送錄影檔堂數
dim bolHavingFreeRecord : bolHavingFreeRecord = false
bolHavingFreeRecord = isHavingFreeRecord(intClientSn, 1, 0, CONST_VIPABC_RW_CONN)
...
<div class="hwork">
    <div class="w">
		...
		if ( 555 = sCInt(int_product_sn) or 567 = sCInt(int_product_sn) ) then
			bolComboHavingRecord = true
...
strSql = " SELECT  * "  '分頁語法
strSql = strSql & " FROM    ( SELECT    client_attend_list_2.sn /* 0 */ ,                                                                                                                               "
...
strSql = strSql & "                     ISNULL(ClientAttendListDetail.SessionPeriod, 45) AS SessionPeriod /* 19 */,                                                                    "
strSql = strSql & "                     ROW_NUMBER() OVER ( ORDER BY client_attend_list_2.sdate DESC, client_attend_list_2.stime DESC ) AS row " '分頁語法
strSql = strSql & "           FROM      ( SELECT    client_attend_list_1.sn ,                                                                                                                               "
strSql = strSql & "                                 client_attend_list_1.client_sn ,                                                                                                                                "
strSql = strSql & "                                 client_attend_list_1.mtel ,                                                                                                                                        "
strSql = strSql & "                                 client_attend_list_1.session_sn ,                                                                                                                             "
strSql = strSql & "                                 client_attend_list_1.situation ,                                                                                                                                "
strSql = strSql & "                                 material.ltitle AS title ,                                                                                                                                              "
strSql = strSql & "                                 con_basic.basic_fname + ' '                                                                                                                                     "
strSql = strSql & "                                 + con_basic.basic_lname AS cname ,                                                                                                                       "
strSql = strSql & "                                 client_attend_list_1.sdate ,                                                                                                                                      "
strSql = strSql & "                                 client_attend_list_1.stime ,                                                                                                                                      "
strSql = strSql & "                                 client_attend_list_1.slevel ,                                                                                                                                     "
strSql = strSql & "                                 cfg_session_points.point_fee AS sfee                                                                                                                      "
strSql = strSql & "                       FROM      ( SELECT    sn ,                                                                                                                                                    "
strSql = strSql & "                                             client_sn ,                                                                                                                                                      "
strSql = strSql & "                                             CONVERT(VARCHAR, attend_date, 111) AS sdate ,                                                                                     "
strSql = strSql & "                                             attend_sestime AS stime ,                                                                                                                            "
strSql = strSql & "                                             attend_mtl_1 AS mtel ,                                                                                                                                 "
strSql = strSql & "                                             session_sn AS session_sn ,                                                                                                                          "
strSql = strSql & "                                             session_state AS situation ,                                                                                                                          "
strSql = strSql & "                                             attend_consultant AS consultant ,                                                                                                                "
strSql = strSql & "                                             attend_level AS slevel ,                                                                                                                                "
strSql = strSql & "                                             attend_livesession_types AS stype                                                                                                              "
strSql = strSql & "                                   FROM      client_attend_list                                                                                                                                    "
strSql = strSql & "                                   WHERE     client_sn = '" & intClientSn & "'                                                                                                             "
strSql = strSql & "                                             AND refund IN ( 0, 1 )                                                                                                                                    "
strSql = strSql & "                                             AND attend_mtl_1 IS NOT NULL                                                                                                                   "
strSql = strSql & "                                             AND valid = 1                                                                                                                                                "
strSql = strSql & "                                             AND session_sn IS NOT NULL                                                                                                                      "
strSql = strSql & "                                 ) AS client_attend_list_1                                                                                                                                          "
strSql = strSql & "                                 LEFT OUTER JOIN cfg_session_points ON client_attend_list_1.stype = cfg_session_points.sn                            "
strSql = strSql & "                                 LEFT OUTER JOIN con_basic ON client_attend_list_1.consultant = con_basic.con_sn                                          "
strSql = strSql & "                                 LEFT OUTER JOIN material ON client_attend_list_1.mtel = material.course                                                          "
strSql = strSql & "                     ) AS client_attend_list_2                                                                                                                                                      "
strSql = strSql & "                     LEFT OUTER JOIN client_session_evaluation ON client_attend_list_2.client_sn = client_session_evaluation.client_sn "
strSql = strSql & "                                                               AND client_attend_list_2.session_sn = client_session_evaluation.session_sn                       "
strSql = strSql & "                     LEFT OUTER JOIN client_homework ON client_attend_list_2.session_sn = client_homework.session_sn                        "
strSql = strSql & "                                                        AND client_attend_list_2.client_sn = client_homework.client_sn                                                   "
strSql = strSql & " 					LEFT OUTER JOIN  ClientAttendListDetail ON ClientAttendListDetail.ParentSn =  client_attend_list_2.sn                         "         
strSql = strSql & "         ) a                                                                                                                                                                                                      "
strSql = strSql & " WHERE   row BETWEEN @top AND @bottom " '分頁語法
strSql = strSql & " ORDER BY a.sdate DESC ,                                                                                                                                                                         "
strSql = strSql & "         a.stime DESC                                                                                                                                                                                     "
...
if ( isSelectQuerySuccess(arrSqlResult) ) then
	...
	if ( Ubound(arrSqlResult) >= 0 ) then
		...
		for intI = 0 to intTotal
            ...
            dim intSessionPeriod : intSessionPeriod = arrSqlResult(19,intI)
%>
            <div class="onebox">
            ...
            if ( isSelectQuerySuccess(arrRecordResult) ) then
                if ( Ubound(arrRecordResult) >= 0 ) then
	                ...
	                if ( Not isEmptyOrNull(strRecordFileName) ) then 
                        ...
                        '20130527 阿捨新增 動態新增錄影檔扣點變數
                        Execute("dim intCostSession : intCostSession = CONST_PRODUCT_MY_RECORD_"& intSessionPeriod &"MINS_POINTS")
                        if ( true = bolDebugMode ) then
                            response.Write "intCostSession:" & intCostSession & "<br />"
                        end if
                        ...
                        if ( ( true = bol1On4Session AND sCInt(intProductStatus)<> 3 ) OR true = bolTestCase) then
		                ...
		                '20121207 阿捨新增 套餐產品 沒有錄影檔權限
                        if ( true = bolComboProduct AND false = bolComboHavingRecord AND false = bolHavingFreeRecord ) then
                        else
%>
                    <span class="bold">
                        <%=strSessionTitle%>
