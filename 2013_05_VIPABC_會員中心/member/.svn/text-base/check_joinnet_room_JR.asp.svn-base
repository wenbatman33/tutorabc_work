...
'if ( intNowHour = 0 AND intNowMins <= CONST_DEFAULT_FINISH_CLASS_MINUTE ) then
	...
	str_sql = "SELECT CONVERT(VARCHAR(8), member.ClassRecord.ClassStartDate, 112) + RIGHT('00' + ISNULL(CONVERT(VARCHAR(2), member.ClassRecord.ClassStartTime),''),2) " 
    str_sql = str_sql & " + RIGHT('000' + ISNULL(CONVERT(VARCHAR(3), member.ClassRecord.ClassRoomNumber),''),3) AS session_sn, session_room_status_1.status " 
	str_sql = str_sql & " FROM member.ClassRecord WITH (NOLOCK) "   
	str_sql = str_sql & " LEFT OUTER JOIN WEB_OFFICE.web_office.dbo.session_room_status AS session_room_status_1 "
    str_sql = str_sql & " ON member.ClassRecord.ClassRoomNumber = session_room_status_1.room "   
	str_sql = str_sql & " WHERE (member.ClassRecord.client_sn = @g_var_client_sn) AND (member.ClassRecord.ClassStartTime = @hstr) "
	str_sql = str_sql & " AND (member.ClassRecord.ClassStatusId = 2 OR member.ClassRecord.ClassStatusId = 3)  AND member.ClassRecord.ClassRecordValid = 1 AND (CONVERT(VARCHAR, member.ClassRecord.ClassStartDate, 111) = CONVERT(VARCHAR, '" & strTodayDate & "', 111)) "
'else
	'str_sql = "SELECT client_attend_list.session_sn, session_room_status_1.status " 
	'str_sql = str_sql & " FROM client_attend_list(nolock) " 
	'str_sql = str_sql & " LEFT OUTER JOIN WEB_OFFICE.web_office.dbo.session_room_status AS session_room_status_1 "   
...
sql="SELECT url, status FROM session_room_status WITH (nolock) WHERE (room = '"&right(session_sn,3)&"')"
Dim g_arr_result
