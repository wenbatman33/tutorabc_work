...
'if ( "1" = strTransBeijingDomain ) then
    'strBankPostUrl = "http://www.vipabc.com/program/member/down_payment.asp"
'else
    strBankPostUrl = "http://payment.chinapay.com/pay/TransGet"
'end if
...
Dim int_comp_type : int_comp_type = CONST_BANK_PLATFORM_PARAMETER_CHINAPAY '1=網際威信 2=華南銀行 3=陸網銀聯 4=支付寶

if ( true = bolBJChannel ) then
    int_comp_type = CONST_BANK_PLATFORM_PARAMETER_BJ_CHINAPAY
	'strBankPostUrl = "http://payment-test.chinapay.com/pay/TransGet"
elseif ( true = bolSHChannel ) then
    int_comp_type = CONST_BANK_PLATFORM_PARAMETER_CHINAPAY
end if

Dim str_MerId : str_MerId = CONST_CREDIT_CARD_TYPE_VIPABC_CHINAPAY_CODE
Dim str_sub_MerId : str_sub_MerId = CONST_CREDIT_CARD_TYPE_VIPABC_CHINAPAY_SUB_CODE 'MerId为ChinaPay统一分配给商户的商户号，15位长度，必填
...
if ( request("change_bank_for_chinapay") = CONST_CREDIT_CARD_TYPE_VIPABC_CHINAPAY ) then
	'內卡
	str_MerId = CONST_CREDIT_CARD_TYPE_VIPABC_CHINAPAY_CODE
	str_sub_MerId = CONST_CREDIT_CARD_TYPE_VIPABC_CHINAPAY_SUB_CODE	
...
elseif ( request("change_bank_for_chinapay") = CONST_CREDIT_CARD_TYPE_VIPABC_UNIONPAY ) then
	'外卡
	str_MerId = CONST_CREDIT_CARD_TYPE_VIPABC_UNIONPAY_CODE
	str_sub_MerId = CONST_CREDIT_CARD_TYPE_VIPABC_UNIONPAY_SUB_CODE	
...
if ( true = bolBJChannel ) then
	str_ChkValue = getNetPaySignBJCode(str_MerId,str_tride_order,str_amount,str_CuryId,str_TransDate,str_TransType,"VIPABC",int_credit_card_type)
else
...
'----------------- 刷卡傳hidden值到下一頁所需要的變數 ----------------------- end ------------------
%>
    <form name="SendToMer" action="<%=strBankPostUrl%>" method="POST">
    ...
    <input type="hidden" name="TransAmt" value="<%=str_amount%>" />
    <!-- 订单交易金额，12位长度，左补0，必填,单位为分 -->
    <input type="hidden" name="CuryId" value="<%=str_CuryId%>" />
    <!-- 订单交易币种，3位长度，固定为人民币156，必填 -->
    <input type="hidden" name="TransDate" value="<%=str_TransDate%>" />
    <!-- 订单交易日期，8位长度，必填 -->
    <input type="hidden" name="TransType" value="<%=str_TransType%>" />
    <!-- 交易类型，4位长度，必填 -->
    ...
    <input type="hidden" name="PageRetUrl" value="http://<%=CONST_VIPABC_WEBHOST_NAME%>/program/member/payment_return_page.asp?score=pageRet" />
    <!-- 页面交易接收URL，长度不要超过80个字节，必填 -->
    <input type="hidden" name="GateId" value="<%=str_GateId%>">
    <!-- 支付网关号，可选 -->
    <input type="hidden" name="Priv1" value="VIPABC">
    <!-- 商户私有域，长度不要超过60个字节 -->
