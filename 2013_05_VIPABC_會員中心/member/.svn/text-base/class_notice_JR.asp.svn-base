...
dim str_attend_sn
        ...
        str_sql = " SELECT TOP 1 ClassRoomNumber, ISNULL(ProfileLearnLevel,0) AS attend_level, ClassStartDate,  ClassStartTime,  ClassRecordId" 
		str_sql = str_sql & " FROM member.ClassRecord WITH (NOLOCK) "   
		str_sql = str_sql & " WHERE client_sn = @client_sn AND ClassStartDate = @ClassStartDate AND ClassStartTime = @ClassStartTime "   
		str_sql = str_sql & " ORDER BY ClassStartDate, ClassStartTime "
		...
		if (isSelectQuerySuccess(arr_result)) then
			...
			if (Ubound(arr_result) >= 0) then
                ...
                str_room = arr_result(0,0)
				str_attend_level = arr_result(1,0)
                str_attend_sn = arr_result(4,0)
...
<!--
    ...
    function onclickchImage(p_obj_img_none, p_str_img_ok_id) {
        ...
        if (int_chk_passed_count >= 3) {
            ...
            if (int_room_num <= 550) {
                window.location.href = "go_joinnet_room_JR.asp"
            }
            else {
                window.location.href = "go_tutormeet_room_JR.asp"
...
arrParam = Array(3, str_attend_sn)
...
arrSqlResult = excuteSqlStatementWrite(strSql, arrParam, CONST_VIPABC_RW_CONN)
