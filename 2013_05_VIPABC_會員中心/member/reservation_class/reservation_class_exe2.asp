...
response.write "<script>alert('str_total_reservation_class_dt = " & str_total_reservation_class_dt & "');</script>"
response.write "<script>alert('str_total_cancel_class_sn = " & str_total_cancel_class_sn & "');</script>"
                ...
                '允許取消
                    ...
                    if (IsArray(arr_cancel)) then
                        ...
                        '取消成功
							...
							'***************新增LOG檔紀錄訂課取消 edit by fox 20130304**************
                            recordpoint(now() & "	" & Request.ServerVariables("REMOTE_ADDR") & "[" & int_client_sn & "] 取消成功隱藏版 [" & strClassType & "][" & Trim(str_arr_total_cancel_class_sn(i)) & "]")
                            '***********************************************************************
                        end if
            ...
            if (Len(Trim(str_reservation_class_datetime)) < 12) then
                ...
                if (true = bol_enabled_set_class) then
                    ...
                    if ( true = bolComboProduct  AND true = bol_enabled_set_class ) then
                        ...
                        dim intRealSessionPoint : intRealSessionPoint = 1
					    '可上堂數
                ...
                if (true = bol_enabled_set_class) then                
                    ...
                    if (Not IsArray(arr_reservation_result)) then
                        ...
                        if (arr_reservation_result(1) = CONST_FUNC_EXE_SUCCESS) then
							...
							'***************新增LOG檔紀錄訂課 edit by fox 20130304**************
                            recordpoint(now() & "	" & Request.ServerVariables("REMOTE_ADDR") & "[" & int_client_sn & "] 新增成功隱藏版 [" & strClassType & "][" & str_reservation_class_datetime & "]")
                            '***********************************************************************																  
...
%>
<%
function recordpoint(word)
Set fs = Server.CreateObject("Scripting.FileSystemObject")
file_Path = Server.MapPath("Evalution_log_" & Year(Now()) & Right("0" & Month(Now()), 2) & Right("0" & Day(Now()), 2) & ".txt")
'1 開啟成為讀的檔案預設值為1 
'2 開啟成唯寫的檔案原檔案內容會被清除 
'8 開啟成唯寫的檔案原檔案內如保留資料會從檔案最後面寫入 
'Set CounterFile = fs.OpenTextFile( file_Path, 8 )
'CounterFile.WriteLine(word)
'CounterFile.Close
'response.write file_Path & "<br/>"
if (fs.FileExists(file_Path) = false) then
    fs.CreateTextFile(file_Path)
