            ...
            if (Ubound(arr_attend_list_data) >= 0) then
                for i = 0 to Ubound(arr_attend_list_data, 2)
                    ...
                    if (DateDiff("n", now(), str_cancel_class_datetime) >= 240 and bol_client_have_valid_contract) then
                        arr_now_td(Day(arr_attend_list_data(0, i))) = arr_now_td(Day(arr_attend_list_data(0, i))) & "<div id=""cancel_class_info_"&arr_attend_list_data(7, i)&""" class="""" style=""float:left; margin-left:10px""><span "&str_ball_style_class&" title=""" & str_title & """>●</span><span class=""color_1"" title=""" & str_title & """>" & arr_attend_list_data(1, i) & ":" & intSTime & "</span>"
                    ...
                    '四小時內無法取消 不顯示取消的按鈕 或 合約已到期
                        ...
                        arr_now_td(Day(arr_attend_list_data(0, i))) = arr_now_td(Day(arr_attend_list_data(0, i))) & "<div id=""cancel_class_info_"&arr_attend_list_data(7, i)&""" class="""" style=""float:left; margin-left:10px""><span "&str_ball_style_class&" title=""" & str_title & """>●</span><span class=""color_1"" title=""" & str_title & """>" & arr_attend_list_data(1, i) & ":" & intSTime & "</span></div>"
                    end if
