    ...
    <div class="ok">
        <%
            ...
            if (Trim(Request("total_order_count")) <> "0") then
                Response.Write str_msg_font_order_success_4 & "<br/>"   '您已成功重新预订
                Response.Write "<span class=""bold"">" & Trim(unEscape(Request("success_msg"))) & "</span>" & str_msg_font_order_success_2 & Trim(Request("total_order_count")) & str_msg_font_order_success_3    '課程共 XX 堂
            end if

            '失敗的訊息
            if (Not isEmptyOrNull(Trim(unEscape(Request("fault_msg"))))) then
                Response.Write "<br/>" & "<span class=""bold"">" & Trim(unEscape(Request("fault_msg"))) & "</span>"
            end if
        %>
