...
'
dim intClassRecordLevel : intClassRecordLevel = 0 '客戶等級
dim strClassRecordCostPoints : strClassRecordCostPoints = "" '扣點數
...
dim strClassType : strClassType = ""
    ...
    '20130318 Create 傳入參數
    if ( not isEmptyOrNull(request("profileId")) ) then
        intProfileId = request("profileId")
    else
        intProfileId = 0
    end if
    if ( not isEmptyOrNull(request("contractId")) ) then
        intContractId = request("contractId")
    else
        intContractId = 0
    end if
            ...
            if (Not isEmptyOrNull(str_arr_total_cancel_class_sn(i)) and _
                ...
                if (Not IsArray(arr_allow_cancel)) then
                    ...
                    '允許取消
                        ...
                        'if (str_arr_total_cancel_class_type(i) <> CONST_CLASS_TYPE_LOBBY) then
                        ...
                        'end if
                        '取得要扣的堂數
                        flt_subtract_session = getSubtractSession(Array(str_class_datetime, str_arr_total_cancel_class_type(i), str_now_use_product_sn, int_special_sn), int_db_connect)
                        ...
                        '*****************************************************************************************/
                        'todo 上線要改為0
                        'arr_cancel = cancelNewClassRecord(Trim(str_arr_total_cancel_class_sn(i)),  str_arr_total_cancel_class_type(i), flt_subtract_session, str_arr_total_cancel_class_date(i), str_arr_total_cancel_class_time(i), int_client_sn, str_now_use_contract_sn, 0)
                        arr_cancel = cancelNewClassRecord(Trim(str_arr_total_cancel_class_sn(i)),  str_arr_total_cancel_class_type(i), flt_subtract_session, str_arr_total_cancel_class_date(i), str_arr_total_cancel_class_time(i), int_client_sn, str_now_use_contract_sn, 1)
                        '舊
                        'arr_cancel = cancelClass(Trim(str_arr_total_cancel_class_sn(i)), int_client_sn, int_db_connect)
            ...
            if (Not isEmptyOrNull(str_reservation_class_datetime)) then
                ...
                if (Len(Trim(str_reservation_class_datetime)) < 12) then
                    ...
                    if (true = bol_enabled_set_class) then                
                        ...
                        'function reservationNewClassRecord(ByVal p_strClassInfo, ByVal p_arrData, ByVal p_intClassType, ByVal p_intClient_sn, ByVal p_intContract_sn, ByVal p_intTestMode)
                        '/****************************************************************************************
                        '描述      ：預訂課程
                        '傳入參數  ：
                        '[p_strClassInfo:String] '已預訂的課程，欲取消的課程，以逗點隔開多筆 
                        '一對一和一對多格式: class_type + date + time 例:072009120820 代表一對一12/8號晚上八點的課
                        '大會堂格式: class_type + date + time + lobby_session.sn
                        '[p_arrData:Array]
                        '[0] = 產品編號
                        '[1] = 扣堂數
                        '[2] = 客戶的等級
                        '[3] = client_website 'T' or 'C'
                        '牽涉數據表：
                        '備註      ：
                        '歷程(作者/日期/原因) ：[ArTHurWu] 2013/04/06 Created
                        '*****************************************************************************************/
                        'todo 上線要改為0
                        'arr_reservation_result = reservationNewClassRecord(str_reservation_class_datetime, Array(str_now_use_product_sn, flt_subtract_session, int_client_attend_level), int_attend_type_tmp, int_client_sn, str_now_use_contract_sn, 0)
                        arr_reservation_result = reservationNewClassRecord(str_reservation_class_datetime, Array(str_now_use_product_sn, flt_subtract_session, int_client_attend_level), int_attend_type_tmp, int_client_sn, str_now_use_contract_sn, 1)
                        '舊
