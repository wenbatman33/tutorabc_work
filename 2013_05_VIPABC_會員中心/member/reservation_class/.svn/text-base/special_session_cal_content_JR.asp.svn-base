...
'                                    '"      OR (CONVERT(varchar, client_attend_list.attend_date, 111) > CONVERT(varchar, GETDATE(), 111))) "
 ...
 '                              "      OR (CONVERT(varchar, client_attend_list.attend_date, 111) > CONVERT(varchar, GETDATE(), 111))) "
str_attend_condition_datetime = " AND ((CONVERT(VARCHAR, member.ClassRecord.ClassStartDate, 111) = CONVERT(VARCHAR, GETDATE(), 111) AND member.ClassRecord.ClassStartTime = '"&Hour(now)&"' AND " & Minute(now) & " < 25 ) "
str_attend_condition_datetime = str_attend_condition_datetime & " OR (CONVERT(VARCHAR, member.ClassRecord.ClassStartDate, 111) = CONVERT(VARCHAR, GETDATE(), 111) AND member.ClassRecord.ClassStartTime > '"&Hour(now)&"' ) "
str_attend_condition_datetime = str_attend_condition_datetime & "      OR (CONVERT(VARCHAR, member.ClassRecord.ClassStartDate, 111) > CONVERT(VARCHAR, GETDATE(), 111))) "
 ...
 if (Not isEmptyOrNull(str_search_con)) then
     ...
     str_attend_condition_con = " AND (member.ClassRecord.ClassConsultantId = '"&str_search_con &"')  "
 end if
...
strSql = strSql & " CASE ISNULL(client_order_lobby.ClassStatusId, 0) WHEN 2 THEN 1 ELSE 0 END AS status, "
'--該堂大會堂的等級 7
'strSql = strSql & " ISNULL(CAST(client_order_lobby.attend_level AS varchar(5)), 0) AS lev, "
strSql = strSql & " ISNULL(CAST(client_order_lobby.ProfileLearnLevel AS varchar(5)), 0) AS lev, "
'--大會堂人數上限 8
...
strSql = strSql & " client_order_lobby.ClassRecordId AS client_attend_list_sn, "
'--顧問名字 fname 15
...
strSql = strSql & " client_order_lobby.ClassTypeId AS attend_livesession_types, "
'--課程教材 18  <!--Allison 新增-->
strSql = strSql & " lobby_session.material , "
'--新增session_type判別一對一/一對多
'strSql = strSql & " client_order_session.session_type , "
strSql = strSql & " client_order_session.ClassTypeId AS session_type, "
'--新增special_sn
'strSql = strSql & " client_order_lobby.special_sn , "
strSql = strSql & " client_order_lobby.LobbySessionId AS special_sn, "
'--分頁語法
...
'strSql = strSql & " ON client_order_lobby.LobbySessionId = lobby_session.sn "
strSql = strSql & " SELECT ClassStatusId, ISNULL(ProfileLearnLevel,0) AS ProfileLearnLevel, LobbySessionId, ClassRecordId, ClassTypeId, "
'--新增status
strSql = strSql & " CASE ISNULL(ClassStatusId, 0) WHEN 2 THEN 1 ELSE 0 END AS status "
strSql = strSql & " FROM member.ClassRecord "
strSql = strSql & " WHERE (client_sn = @client_sn) AND (ClassStatusId = 2) AND LobbySessionId > '' " & str_attend_condition_datetime & str_attend_condition_con & str_attend_condition_time
strSql = strSql & " ) AS client_order_lobby "
'strSql = strSql & " ON client_order_lobby.attend_level = lobby_session.sn "
...
strSql = strSql & " SELECT CONVERT(VARCHAR, ClassStartDate, 112) + RIGHT('0'+CAST(ClassStartTime AS VARCHAR(2)),2) AS date_time, ClassRecordId, ClassTypeId  "
strSql = strSql & " FROM member.ClassRecord "
strSql = strSql & " WHERE (client_sn = @clientsn ) AND (ClassStatusId = 2) AND LobbySessionId IS NULL " & str_attend_condition_datetime & str_attend_condition_con & str_attend_condition_time
strSql = strSql & " ) AS client_order_session "
strSql = strSql & " ON client_order_session.date_time = lobby_session.date_time "
...
'字典檔 給值
 if ( not objLobbySessionData.eof ) then
    while not objLobbySessionData.eof
        ...
        dim arrClassTypeVIPJR : arrClassTypeVIPJR = null
        dim arrClassTypeVIPABC : arrClassTypeVIPABC = null
        ...
        for intI = 0 to Ubound(arrClassTypeVIPJR) 
            if ( sCInt(intSessionType) = sCInt(arrClassTypeVIPJR(intI)) ) then
                intSessionType = arrClassTypeVIPABC(intI)
                exit for
            end if
        next
        ...
        if ( strKeyWord = strTempKeyword ) then
            ...
            arrSessionType(intTempNum) = strSessionType '判斷一對一/一對多
                ...
                if ( true = isArray(arrSessionTopic) ) then '當同時段有多筆資料時
                    ...
                    for intL = 0 to intTotal
                        ...
                        if ( Not isEmptyOrNull(strSessionTopic) ) then
                            ...
                            elseif ( (intNowPage = 2 AND intK >2) OR (4 = sCint(intSessionType) OR 7 = sCint(intSessionType) OR 14 = sCint(intSessionType)) ) then
                            ...
                            elseif ( strKeyWord = strTempKeyword AND true = bolHasReserved ) then '同時段大會堂 已訂課 非第一筆 判斷
                                ...
                                if ( 1 = intSessionStatus ) then 
                                    strShowInfo =  "<div class='r_session " & strChecked & " ' onmouseover=""this.style.backgroundColor='#D1E3ED'"" style='cursor:hand;text-decoration:none;' onclick=""OpenMsg('" & strSessionSDate & "', '" & intSessionSTime & "', '" & strConsultantEName & "', '" & strImageUrl & "', '" & "" &"', '" & "" &"', '" & strTempSessionLv &"', " & intSessionStatus &", '" & intSessionSn &"', '" & intClientAttendSn &"');""><div class='title_c' title='" & strShowMouseOver &"'>"
                                else
                                    strShowInfo =  "<div class='r_session " & strChecked & " ' style='cursor:hand;text-decoration:none;' onclick=""OpenMsg('" & strSessionSDate & "', '" & intSessionSTime & "', '" & strConsultantEName & "', '" & strImageUrl & "', '" & "" &"', '" & "" &"', '" & strTempSessionLv &"', " & intSessionStatus &", '" & intSessionSn &"', '" & intClientAttendSn &"');""><div class='title_c' title='" & strShowMouseOver &"'>"
                                end if
                            end if
