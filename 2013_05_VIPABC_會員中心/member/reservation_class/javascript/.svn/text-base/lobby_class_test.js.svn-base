var g_obj_reservation_lobby_class_ctrl = {
    ...
    arr_page_url: {
        "ajax_lobby_class_info": "/program/member/reservation_class/include/ajax_lobby_class_info_test.asp",        // ajax 動態載入 根據搜尋條件的大會堂資訊
    ...
    addNewLobbyClass: function (p_str_lobby_session_sn, p_bol_from_other_page, p_str_lobby_date, p_str_lobby_time, p_str_con_ename, p_str_lobby_topic, p_str_lobby_sn)
    {
        ...
        {
            var str_post_data = "act=chkorder&rcdate=" + escape(p_str_lobby_date) + "&rctime=" + escape(p_str_lobby_time.substring(0, 2));
            // 檢查客戶是否已經預定了此時段的課程
