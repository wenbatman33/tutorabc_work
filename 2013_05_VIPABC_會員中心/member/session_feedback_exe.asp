...
if currentDateTime < newUIOnlineDateTime then
	...
	if request("C1")="ON" then
		...
		var_arr = Array(str_session_sn, g_var_client_sn, str_consultant, str_materials, str_overall, str_compliment, str_suggestions, str_behavior, str_distribution, str_speed, str_tskill, str_requirement, str_difficulty, str_connection,1,request("ctime"))
	else
		...
		var_arr = Array(str_session_sn, g_var_client_sn, str_consultant, str_materials, str_overall, str_compliment, str_suggestions, str_behavior, str_distribution, str_speed, str_tskill, str_requirement, str_difficulty, str_connection)
	end if
    ...
    '寫feedback_classmate
	var_arr_classmate = Array(g_var_client_sn, str_session_sn, g_var_client_sn)
	str_sql_classmate = "SELECT client_attend_list.client_sn, "
	str_sql_classmate = str_sql_classmate&" classmate_session_feedback.condition, classmate_session_feedback.performance  "
	str_sql_classmate = str_sql_classmate&" FROM client_attend_list "
	str_sql_classmate = str_sql_classmate&" LEFT OUTER JOIN classmate_session_feedback ON client_attend_list.client_sn = classmate_session_feedback.client_sn AND client_attend_list.session_sn = classmate_session_feedback.session_sn and classmate_session_feedback.inputer IN (@client_sn1) "
	str_sql_classmate = str_sql_classmate&" WHERE (client_attend_list.valid = 1) AND (client_attend_list.session_sn = @session_sn) AND (NOT (client_attend_list.client_sn IN (@client_sn2)))  "
	str_sql_classmate = str_sql_classmate&" ORDER BY client_attend_list.attend_room " 
	arr_result_classmate = excuteSqlStatementRead(str_sql_classmate, var_arr_classmate, CONST_VIPABC_RW_CONN)

    if (isSelectQuerySuccess(arr_result_classmate)) then  '有資料
	    if (Ubound(arr_result_classmate) > 0) then
		    for i = 0 to Ubound(arr_result_classmate,2)
			    '濾掉沒填寫的資料
			    var_tmp_condition_value=request("Customer"&arr_result_classmate(0,i)&"_S")&", "&request("Customer"&arr_result_classmate(0,i)&"_S2F")&", "&request("Customer"&arr_result_classmate(0,i)&"_S3F")&", "&request("Customer"&arr_result_classmate(0,i)&"_S4F")&", "&request("Customer"&arr_result_classmate(0,i)&"_S5F")
			    var_tmp_performance_value=request("Customer"&arr_result_classmate(0,i)&"_P")
		
			    if trim(var_tmp_condition_value)<>", , , ," and trim(var_tmp_performance_value)<>"" then 
			    'insert DB
				    ...
				    var_arr = Array(g_var_client_sn, arr_result_classmate(0,i), str_session_sn, var_tmp_condition_value, var_tmp_performance_value )
	...
	if str_consultant="" or str_materials="" or str_overall="" or str_session_sn="" then
		...
		if objPCInfo1.eof then '沒資料代表沒做過評鑑
			...
			sql = sql & "	AND (client_attend_list.session_sn = '"& str_session_sn &"') "
			sql = sql & "	AND (NOT (client_attend_list.client_sn IN ("& g_var_client_sn &"))) "
...
if ( bolDeBugMode ) then
    ...
    Call alertGo(getWord("feedback_send"), "session_feedback_success.asp", CONST_ALERT_THEN_REDIRECT)
end if
%>
