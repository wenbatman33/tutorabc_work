...
<%
	...
	Dim intClientSn : intClientSn = Session("client_sn") '取得客戶編號
...
if Request("status")="save" then '不是從咨詢室進入
	'**** 判斷客戶有無上課 start ****
	str_sql = "SELECT top 1 Class_yesno, session_state FROM dbo.client_attend_list WITH (NOLOCK) WHERE (Session_sn = @session_sn ) AND (client_sn =@client_sn)"
	var_arr = Array(SessionSn,intClientSn)
	arr_result = excuteSqlStatementRead(str_sql , var_arr, CONST_VIPABC_RW_CONN)
	if (isSelectQuerySuccess(arr_result)) then
		if ( Ubound(arr_result) >= 0 ) then 
			'if Class_yesno=0 表示沒有上課，就不給他進來填課後評鑑
			if (arr_result(0,0) = 0 and arr_result(1,0) = 0 ) then 
				bolClassAttend = true
			end if			
		else
			bolClassAttend = true		
	...
	'if (bolClassAttend) then Call alertGo(strLangSessionFeedback00025, "" , CONST_ALERT_THEN_NOTHING )						
	'**** 判斷客戶有無上課 end ****
else '是從咨詢室進入
...
<div id="temp_contant">
    <!--內容start-->
    <div class='page_title_2'><h2 class='page_title_h2'>问题小帮手</h2></div>
  	<div class="box_shadow" style="background-position:-80px 0px;"></div>
    <div id="issue_membox">
<!--问题小帮手 start-->
<div id="dialog-form" title="问题小帮手" /></div>
<!--问题小帮手 end-->
        <form action="session_feedback_exe.asp" name="frm1" id="frm1" method="post" onSubmit="return chkdata();">
        <div><table align="center" width="100%"><tr><td align="center"></td></tr></table></div>
...
sql = "SELECT client_sn "
sql = sql & "FROM client_attend_list "
sql = sql & "WHERE valid = 1 "
sql = sql & "	AND session_sn = '"&SessionSn&"' "
sql = sql & "	AND special_sn IS NULL "
sql = sql & "	AND attend_livesession_types NOT IN (1,7)"
...
<%
		...
		sql = sql & "	 , convert(nvarchar, client_attend_list.Attend_Date,111) Attend_Date "
		sql = sql & "	 , client_attend_list.Attend_Sestime "
		sql = sql & "	 , material.LTitle "
		sql = sql & "	 , material.LD "
		sql = sql & "	 , client_attend_list.Attend_Level "
		sql = sql & "	 , material.Course "
		sql = sql & "	 , con_basic.Con_Sn "
		sql = sql & "FROM client_attend_list with (nolock) "
		sql = sql & "	LEFT OUTER JOIN material with (nolock) "
		sql = sql & "		ON client_attend_list.Attend_mtl_1 = material.Course "
		sql = sql & "	LEFT OUTER JOIN con_basic with (nolock) "
		sql = sql & "		ON client_attend_list.Attend_consultant = con_basic.Con_Sn "
		sql = sql & "	LEFT JOIN client_basic with (nolock) "
		sql = sql & "		ON client_basic.sn = client_attend_list.client_sn "
		sql = sql & "WHERE (client_attend_list.Session_sn = '"&SessionSn&"') "
...
%>
<%
					isnup = false		'記錄是否沒有上課
					Set objInfo = Server.CreateObject("TutorLib.UtilityLib.DBOperation.DBCmdOp")
					sql = "SELECT session_state FROM client_attend_list(nolock) WHERE (client_sn = "&intClientSn&") AND (valid = 1) AND (session_state = '0') AND (session_sn = '"&SessionSn&"')"
					intSqlWriteResult = objInfo.excuteSqlStatementEachQuick(sql, CONST_TUTORABC_R_CONN)
					if not objInfo.eof then
						isnup = true
					end if
...
<%
		...
		sql = " select "
		sql = sql & " isnull(participation,0) participation, isnull(pronunciation,0) pronunciation, "
		'**** 2011/04/26 RD-張亞琴 修改 : 避免欄位c_comments為Null值,導致後續取值replace時候的錯誤 Start ****
		sql = sql & " isnull(comprehension,0) comprehension, isnull(creativity,0) creativity, isnull(facility,0) facility, isnull(c_comments,'') as c_comments,sel "  
		'**** 2011/04/26 RD-張亞琴 修改 : 避免欄位c_comments為Null值,導致後續取值replace時候的錯誤 End ****
		sql = sql & " from ( "
		sql = sql & "	select session_sn,client_sn, participation, pronunciation, "
		sql = sql & "	comprehension, creativity, facility, c_comments "
		sql = sql & "	from Client_Progress_Report "
		sql = sql & "	where session_sn = '"&SessionSn&"' and client_sn = '"&intClientSn&"' "
		sql = sql & "	and not exists( "
		sql = sql & "		select * from ( "
		sql = sql & "		select client_sn from Client_Progress_Report_history "
		sql = sql & " 		where session_sn = '"&SessionSn&"' and client_sn = '"&intClientSn&"' "
		sql = sql & "	)Report_history "
		sql = sql & "	where Client_Progress_Report.client_sn=Report_history.client_sn "
		sql = sql & " ) "
		sql = sql & " union all "
		sql = sql & " select session_sn, client_sn, participation, pronunciation, "
		sql = sql & " comprehension, creativity, facility, c_comments "
		...
		sql = sql & "		where sn = ( "
		sql = sql & "			select max(sn) as sn "
		sql = sql & "			from Client_Progress_Report_history "
		sql = sql & "			where session_sn = '"&SessionSn&"' and client_sn = '"&intClientSn&"' "
		sql = sql & "		) "
		sql = sql & " ) as progress "
		sql = sql & " LEFT OUTER JOIN ( "
		sql = sql & "	SELECT client_trial_process.session_sn, "
		sql = sql & "	CASE isnull(client_trial_process.agree, 3) WHEN 1 THEN '同意' WHEN 0 THEN '不同意' ELSE '' END + '' + CASE client_trial_list.selection WHEN 'Upgrade' THEN '升級' WHEN 'Downgrade' THEN '降級' ELSE '' END AS sel, "
		sql = sql & "	client_trial_list.client_sn FROM client_trial_process "
		sql = sql & "	LEFT OUTER JOIN client_trial_list  "
		sql = sql & "	ON client_trial_process.tsn = client_trial_list.sn "
		sql = sql & "	WHERE (client_trial_list.client_sn = '"&intClientSn&"') AND "
		sql = sql & "	(client_trial_process.session_sn = '"&SessionSn&"') "
		sql = sql & " ) AS derivedtbl_1 "
		sql = sql & " ON progress.client_sn = derivedtbl_1.client_sn "
		sql = sql & " AND progress.session_sn = derivedtbl_1.session_sn  "
		intSqlWriteResult = objInfo.excuteSqlStatementEachQuick(sql, CONST_TUTORABC_R_CONN)
		if not objInfo.eof then
...
<%
		...
		sql = sql & "FROM (SELECT TOP (10) b.consultant_points "
		sql = sql & "	  FROM dbo.client_attend_list AS a "
		sql = sql & "		  INNER JOIN dbo.client_session_evaluation AS b "
		sql = sql & "			  ON a.client_sn = b.client_sn AND a.session_sn = b.session_sn "
		sql = sql & "	  WHERE (a.attend_consultant = '"&con_sn&"') "
		sql = sql & "		  AND (substring(b.session_sn, 1, 10) < '"&Left(request("session_sn"),10)&"') ORDER BY a.session_sn DESC) AS a_1 "
		intSqlWriteResult = objInfo.excuteSqlStatementEachQuick(sql, CONST_TUTORABC_R_CONN)
		consultant = 0
		if not objInfo.eof then
...
<%
		...
		if (isRegularSession) then
			Set objInfo = Server.CreateObject("TutorLib.UtilityLib.DBOperation.DBCmdOp")
			sql = "SELECT client_attend_list.client_sn "
			sql = sql & "	 , isnull(client_basic.fname, '') + ' ' + left(isnull(client_basic.lname, ''), 1) AS cname "
			sql = sql & "	 , classmate_session_feedback.condition "
			sql = sql & "	 , classmate_session_feedback.performance "
			sql = sql & "FROM client_attend_list "
			sql = sql & "	LEFT OUTER JOIN classmate_session_feedback "
			sql = sql & "		ON client_attend_list.client_sn = classmate_session_feedback.client_sn AND client_attend_list.session_sn = classmate_session_feedback.session_sn AND classmate_session_feedback.inputer IN ("&intClientSn&") "
			sql = sql & "	LEFT OUTER JOIN client_basic "
			sql = sql & "		ON client_attend_list.client_sn = client_basic.sn "
			sql = sql & "WHERE (client_attend_list.valid = 1) "
			sql = sql & "	AND (client_attend_list.session_sn = '"&SessionSn&"') "
			sql = sql & "	AND (NOT (client_attend_list.client_sn IN ("&intClientSn&"))) "
			sql = sql & "ORDER BY client_attend_list.attend_room "
			intSqlWriteResult = objInfo.excuteSqlStatementEachQuick(sql, CONST_TUTORABC_R_CONN)
			valueOfClassmate = 1
%>
...
<% 
'20111225 Beson 由"上課記錄&評分"或"賬戶資訊"進入的不修改出席狀態 Start ---
if Request("status")="save" then
else
...
'20111225 Beson 由"上課記錄&評分"或"賬戶資訊"進入的不修改出席狀態 End ---
%>
